...
export class Document {
  ...
  toXML(opts: Document.ToXMLOptions = {}) {
    const _: Document.RequiredToXMLOptions = {...DefaultToXMLOptions, ...opts, source: this.source}
    const step = (_.preferSpaces ? ' ' : '\t').repeat(_.tabSize)

    const nodes = opts.removeComment ? this.nodes.filter(removeComentNode) : this.nodes
