...
export function getWxmlTag(doc: TextDocument, pos: Position): null | Tag {
  ...
  let pureLine = line.replace(/\{\{[^\}]*?\}\}/g, replacer('^'))

  let attrFlagLine = pureLine.replace(/("[^"]*"|'[^']*')/g, replacer('%')) // 将引号中的内容也替换了

  // console.log(pureLine)
  pureLine.replace(TAG_REGEXP, (raw: string, name: string, attrstr: string, index: number) => {
    attrstr = line.substr(index + raw.indexOf(attrstr))
